---
title: "[初识]为什么 RAG 管道会失败？高级 RAG 模式 — 第 1 部分"
description: Explore the reasons why RAG pipelines fail, covering retrieval, augmentation, and generation problems. 探索 RAG 管道失败的原因，涵盖检索、增强和生成问题。
---

# [检索]为什么 RAG 管道会失败？高级 RAG 模式 — 第 1 部分
# [Retrieval] Why do RAG pipelines fail? Advanced RAG Patterns - Part 1
...
## b) Augmentation Problems b） 增强问题
...
## c) Generation problems c） 生成问题

### LLM related problems LLM相关问题
...
- Mikolov, T., Sutskever, I., Chen, K., Corrado, G. S., & Dean, J. (2013). 单词和短语及其组合性的分布式表示。在神经信息处理系统进展中（第 3111-3119 页）。
