...
URL Source: https://huggingface.co/blog/train-reranker
...
[![Image 2: Model size vs NDCG for Rerankers on GooAQ](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/train-reranker/reranker_gooaq_model_size_ndcg.png)](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/train-reranker/reranker_gooaq_model_size_ndcg.png)
...
[](https://huggingface.co/blog/train-reranker#table-of-contents) Table of Contents
----------------------------------------------------------------------------------

*   [What are Reranker models?](https://huggingface.co/blog/train-reranker#what-are-reranker-models)
*   [Why Finetune?](https://huggingface.co/blog/train-reranker#why-finetune)
*   [Training Components](https://huggingface.co/blog/train-reranker#training-components)
*   [Dataset](https://huggingface.co/blog/train-reranker#dataset)
    *   [Data on the Hugging Face Hub](https://huggingface.co/blog/train-reranker#data-on-the-hugging-face-hub)
    *   [Local Data (CSV, JSON, Parquet, Arrow, SQL)](https://huggingface.co/blog/train-reranker#local-data-csv-json-parquet-arrow-sql)
    *   [Local Data that requires pre-processing](https://huggingface.co/blog/train-reranker#local-data-that-requires-pre-processing)
    *   [Dataset Format](https://huggingface.co/blog/train-reranker#dataset-format)
    *   [Hard Negatives Mining](https://huggingface.co/blog/train-reranker#hard-negatives-mining)

*   [Loss Function](https://huggingface.co/blog/train-reranker#loss-function)
*   [Training Arguments](https://huggingface.co/blog/train-reranker#training-arguments)
*   [Evaluator](https://huggingface.co/blog/train-reranker#evaluator)
    *   [CrossEncoderCorrelationEvaluator with STSb](https://huggingface.co/blog/train-reranker#crossencodercorrelationevaluator-with-stsb)
    *   [CrossEncoderRerankingEvaluator with GooAQ mined negatives](https://huggingface.co/blog/train-reranker#crossencoderrerankingevaluator-with-gooaq-mined-negatives)

*   [Trainer](https://huggingface.co/blog/train-reranker#trainer)
    *   [Callbacks](https://huggingface.co/blog/train-reranker#callbacks)
    *   [Multi-Dataset Training](https://huggingface.co/blog/train-reranker#multi-dataset-training)

*   [Training Tips](https://huggingface.co/blog/train-reranker#training-tips)
*   [Evaluation](https://huggingface.co/blog/train-reranker#evaluation)
*   [Additional Resources](https://huggingface.co/blog/train-reranker#additional-resources)
    *   [Training Examples](https://huggingface.co/blog/train-reranker#training-examples)
    *   [Documentation](https://huggingface.co/blog/train-reranker#documentation)

[](https://huggingface.co/blog/train-reranker#what-are-reranker-models) What are Reranker models?
-------------------------------------------------------------------------------------------------
...
[](https://huggingface.co/blog/train-reranker#why-finetune) Why Finetune?
-------------------------------------------------------------------------

Reranker models are often tasked with a challenging problem:

> Which of these k k highly-related documents answers the query the best?
...
In the [Evaluation](https://huggingface.co/blog/train-reranker#evaluation) section end of this blogpost, I'll show that training a model on your domain can outperform any general-purpose reranker model, even if those baselines are much bigger. Don't underestimate the power of finetuning on your domain!

[](https://huggingface.co/blog/train-reranker#training-components) Training Components
--------------------------------------------------------------------------------------

Training reranker models involves the following components:

1.   [**Dataset**](https://huggingface.co/blog/train-reranker#dataset): The data used for training and/or evaluation.
2.   [**Loss Function**](https://huggingface.co/blog/train-reranker#loss-function): A function that measures the model's performance and guides the optimization process.
3.   [**Training Arguments**](https://huggingface.co/blog/train-reranker#training-arguments) (optional): Parameters that impact training performance, tracking, and debugging.
4.   [**Evaluator**](https://huggingface.co/blog/train-reranker#evaluator) (optional): A class for evaluating the model before, during, or after training.
5.   [**Trainer**](https://huggingface.co/blog/train-reranker#trainer): Brings together all training components.

Let's take a closer look at each component.

[](https://huggingface.co/blog/train-reranker#dataset) Dataset
--------------------------------------------------------------
...
### [](https://huggingface.co/blog/train-reranker#data-on-the-hugging-face-hub) Data on the Hugging Face Hub
...
### [](https://huggingface.co/blog/train-reranker#local-data-csv-json-parquet-arrow-sql) Local Data (CSV, JSON, Parquet, Arrow, SQL)
...
### [](https://huggingface.co/blog/train-reranker#local-data-that-requires-pre-processing) Local Data that requires pre-processing
...
### [](https://huggingface.co/blog/train-reranker#dataset-format) Dataset Format
...
1.   All columns not named "label", "labels", "score", or "scores" are considered _Inputs_ according to the [Loss Overview](https://sbert.net/docs/cross_encoder/loss_overview.html) table. The number of remaining columns must match the number of valid inputs for your chosen loss.
...
### [](https://huggingface.co/blog/train-reranker#hard-negatives-mining) Hard Negatives Mining

The success of training reranker models often depends on the quality of the _negatives_, i.e. the passages for which the query-negative score should be low. Negatives can be divided into two types:
...
[](https://huggingface.co/blog/train-reranker#loss-function) Loss Function
--------------------------------------------------------------------------
...
[](https://huggingface.co/blog/train-reranker#training-arguments) Training Arguments
------------------------------------------------------------------------------------
...
### [](https://huggingface.co/blog/train-reranker#crossencodercorrelationevaluator-with-stsb) CrossEncoderCorrelationEvaluator with STSb
...
### [](https://huggingface.co/blog/train-reranker#crossencoderrerankingevaluator-with-gooaq-mined-negatives) CrossEncoderRerankingEvaluator with GooAQ mined negatives
...
[](https://huggingface.co/blog/train-reranker#trainer) Trainer
--------------------------------------------------------------
...
After running this script, the [tomaarsen/reranker-ModernBERT-base-gooaq-bce](https://huggingface.co/tomaarsen/reranker-ModernBERT-base-gooaq-bce) model was uploaded for me. See the upcoming [Evaluation](https://huggingface.co/blog/train-reranker#evaluation) section with evidence that this model outperformed 13 commonly used open-source alternatives, including much bigger models. I also ran the model with [`answerdotai/ModernBERT-large`](https://huggingface.co/answerdotai/ModernBERT-large) as the base model, resulting in [tomaarsen/reranker-ModernBERT-large-gooaq-bce](https://huggingface.co/tomaarsen/reranker-ModernBERT-large-gooaq-bce).
...
### [](https://huggingface.co/blog/train-reranker#callbacks) Callbacks
...
### [](https://huggingface.co/blog/train-reranker#multi-dataset-training) Multi-Dataset Training
...
[](https://huggingface.co/blog/train-reranker#training-tips) Training Tips
--------------------------------------------------------------------------
...
[![Image 4: Model size vs NDCG for Rerankers on GooAQ](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/train-reranker/reranker_gooaq_model_size_ndcg.png)](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/train-reranker/reranker_gooaq_model_size_ndcg.png)
...
Don't underestimate the power of finetuning reranker models on your domain. You can improve both the search performance _and_ the latency of your search stack by finetuning a (small) reranker!

[](https://huggingface.co/blog/train-reranker#additional-resources) Additional Resources
----------------------------------------------------------------------------------------

### [](https://huggingface.co/blog/train-reranker#training-examples) Training Examples
...
### [](https://huggingface.co/blog/train-reranker#documentation) Documentation
